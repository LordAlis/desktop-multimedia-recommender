...
class MediaRepository:
    def __init__(self, connection: sqlite3.Connection) -> None:
        ...
        # Migrate DBs created before the language column existed so inserts don't fail.
        cols = [r[1] for r in self._conn.execute("PRAGMA table_info(media_items)")]
        if "language" not in cols:
            self._conn.execute("ALTER TABLE media_items ADD COLUMN language TEXT")
        self._conn.commit()
        ...
        self._conn.execute(
            ...
            INSERT OR REPLACE INTO media_items
              (id, title, media_type, description, release_year,
               genres_json, moods_json, source, external_id, rating, popularity, language)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                item.id, item.title, item.media_type, item.description,
                item.release_year, json.dumps(item.genres), json.dumps(item.moods),
                item.source, item.external_id, item.rating, item.popularity, item.language,
            ),
        )
        self._conn.commit()
